...
/**
 * POST /api/courses/{id}/content-nodes — Create a Week/Module/Topic node
 * FR-2.2
 */
@Data @NoArgsConstructor @AllArgsConstructor
public class CreateContentNodeRequest {

    @NotNull(message = "Node type is required")
    private ContentNode.NodeType nodeType;  // WEEK, MODULE, TOPIC

    @NotBlank(message = "Node title is required")
    @Size(max = 200)
    private String title;

    // Required for MODULE and TOPIC (their parent id)
    // null = WEEK (top-level)
